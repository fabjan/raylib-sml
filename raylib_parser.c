// Modified version of the original raylib_parser.c from Raylib 5.5 extended to generate SML bindings.
// Modifications are made by Alexey Kutepov (@rexim)
...
/**********************************************************************************************
    ...
    Copyright (c) 2026      Alexey Kutepov (@rexim)
...
typedef enum { DEFAULT = 0, JSON, XML, LUA, CODE, SML } OutputFormat;
...
int main(int argc, char* argv[])
{
    ...
    else if (outputFormat == SML) printf("\nOutput format:    SML\n\n");
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        else if (IsTextEqual(argv[i], "-f", 2) || IsTextEqual(argv[i], "--format", 8))
        {
            if (((i + 1) < argc) && (argv[i + 1][0] != '-'))
            {
                ...
                else if (IsTextEqual(argv[i + 1], "SML\0", 4)) outputFormat = SML;
            }
...
static void ExportParsedData(const char *fileName, int format)
{
    ...
    switch (format)
    {
        ...
        case SML: {
            const char *dll_name = "./libmraylib.so";
            FILE *outCFile = fopen(temp_sprintf("%s.c", fileName), "wt");

            typedef struct {
                const char *c_type;
                const char *sml_type;
                const char *value_to_c;
                const char *c_to_value;
            } Type;
            ...
            static Type types[] = {
                {
                    .c_type     = "const char *",
                    .sml_type   = "string",
                    .value_to_c = "String_val",
                    .c_to_value = "Val_string",
                },
                {
                    .c_type     = "void",
                    .sml_type   = "unit",
                ...
                },
                {
                    .c_type     = "float",
                    .sml_type   = "real",
                    .value_to_c = "Double_val",
                    .c_to_value = "copy_double",
                },
                {
                    .c_type     = "unsigned int",
                    .sml_type   = "word",
            ...
            Type *get_c_type(const char *c_type)
            {
                for (size_t i = 0; i < ARRAY_LEN(types); ++i) {
                    if (strcmp(c_type, types[i].c_type) == 0) {
                        return &types[i];
                    }
                }
                return NULL;
            ...
            const char *translate_type_c_to_sml(const char *c_type)
            {
                Type *type = get_c_type(c_type);
                if (type == NULL) return temp_sprintf("UNKNOWN[%s]", c_type);
                return type->sml_type;
            }
            ...
            // C header
            {
                ...
                fprintf(outCFile, "{\n");
                fprintf(outCFile, "    int len = strlen(s);\n");
                fprintf(outCFile, "    value res = alloc_string(len);\n");
                fprintf(outCFile, "    bcopy(s, &Byte(res, 0), len);\n");
                ...
                fprintf(outCFile, "}\n");
                fprintf(outCFile, "\n");
                fprintf(outCFile, "static inline value Val_double(double x)\n");
                fprintf(outCFile, "{\n");
            ...
            for (int i = 0; i < funcCount; i++) {
                ...
                for (int p = 0; !skip_for_now && p < funcs[i].paramCount; p++) {
                    ...
                    if (type == NULL || type->value_to_c == NULL || type->c_to_value == NULL) {
                        skip_for_now = true;
                    }
                }
                ...
                // SML
                {
                    ...
                    fprintf(outFile, "val %s: ", funcs[i].name);
                ...
                // C
                {
                    if (skip_for_now) fprintf(outCFile, "/*\n");
                    ...
                    fprintf(outCFile, ");\n");
                    if (strcmp(funcs[i].retType, "void") != 0) {
                        Type *type = get_c_type(funcs[i].retType);
                        const char *c_to_value = type == NULL || type->c_to_value == NULL ? "UNKNOWN" : type->c_to_value;
                        fprintf(outCFile, "    return %s(result);\n", c_to_value);
                    } else {
                        fprintf(outCFile, "    return Val_unit;\n");
                    }
                    fprintf(outCFile, "}\n");
                    if (skip_for_now) fprintf(outCFile, "*/\n");
                    fprintf(outCFile, "\n");
            ...
            size_t total = generated + skipped;
            printf("generated function %zu/%zu (%zu%%)\n", generated, total, generated*100/total);
